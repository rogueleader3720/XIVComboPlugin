            ...
            if (HasEffect(MNK.Buffs.FormlessFist))
            {
                switch (actionID)
                {
                    ...
                    case MNK.PerfectBalance:
                        return (LocalPlayer?.TargetObject is not null && GetCooldown(MNK.Demolish).CooldownRemaining < 0.5) || !IsEnabled(CustomComboPreset.MonkSTComboDemolishOption) ? MNK.Demolish : actionID;
                    case MNK.FormShift:
