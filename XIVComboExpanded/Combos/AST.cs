    ...
    internal class AstrologianCardsOnDrawFeature : CustomCombo
    {
        ...
        protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
        {
            if (actionID == AST.Play)
            {
                ...
                if (CanUseAction(AST.Astrodyne) && IsEnabled(CustomComboPreset.AstrologianAstrodynePlayFeature) && (gauge.DrawnCard != CardType.NONE || GetCooldown(AST.Draw).CooldownRemaining > 30))
